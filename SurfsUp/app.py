...
def welcome():
    ...
    return (
        ...
        f"Temperature stat from the start date: /api/v1.0/start(yyyy-mm-dd)<br/>"
        f"Temperature stat from start to end dates: /api/v1.0/start/end(yyyy-mm-dd)<br/>"
    )
...
def precipitation():
    ...
    queryresult = session.query(*sel).filter(Measurement.date >= last_year).all()             
